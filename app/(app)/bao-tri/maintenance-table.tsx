...
export function MaintenanceTable({ schedules }: { schedules: ScheduleRow[] }) {
  const [q, setQ] = useState("");
  const filtered = useMemo(
    () =>
      schedules.filter((s) =>
        matchesQuery(`${s.name} ${s.scope === "unit" ? s.unitName ?? "" : "Toàn nhà"}`, q),
      ),
    [schedules, q],
  );
  ...
  return (
    <div>
      <SearchBox value={q} onChange={setQ} placeholder="Tìm theo công việc hoặc phạm vi…" />
            ...
            {filtered.map((s) => {
              ...
              return (
                <tr key={s.id} className="border-b border-line last:border-0">
                  ...
                  <td className="px-4 py-3 text-center text-muted">{s.scope === "unit" ? s.unitName : "Toàn nhà"}</td>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-4 text-center text-muted">
                  {schedules.length === 0 ? "Chưa có lịch bảo trì." : "Không tìm thấy lịch phù hợp."}
                </td>
              </tr>
            )}
          </tbody>
